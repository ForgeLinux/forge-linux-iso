...
old_buildiso=$(grep -oP "forgelinuxVersion='v\d{2}\.\d{2}\.\d{2}'" installation-scripts/build-the-iso.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
old_profiledef=$(grep -oP 'forgelinux-v\d{2}\.\d{2}\.\d{2}' archiso/profiledef.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
...
# Replace entire forgelinuxVersion='...' line
sed -i "s|\(.*forgelinuxVersion='\)[^']*\('.*\)|\1$newversion\2|" installation-scripts/build-the-iso.sh

# Replace entire iso_label="forgelinux-..." line
sed -i "s|^iso_label=\"forgelinux-.*\"|iso_label=\"forgelinux-$newversion\"|" archiso/profiledef.sh
